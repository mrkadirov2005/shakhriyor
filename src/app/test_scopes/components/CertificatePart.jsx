...
export default function CertificatePart() {
    const test_type=localStorage.getItem("test_type").split("/")[1]
  ...
  (
    ...
    <Link href={"/"}>❌</Link>
        ...
        <div id='certificate' className="w-screen h-screen flex items-center justify-center bg-gray-100">
  <div className="bg-white shadow-xl rounded-lg p-10 w-[800px] h-[600px] border-4 border-yellow-500 text-center relative">
    
    <h1 className="text-4xl font-bold text-yellow-600">Level Completion Certificate</h1>
    <p className="text-gray-600 text-lg mt-2">This is awarded to</p>
    
    <h2 className="text-3xl font-semibold mt-4 text-gray-800" id="userName">{name} {lastName}</h2>

    <p className="text-gray-600 text-lg mt-4">
      for successfully completing <span className="font-semibold text-yellow-500" id="levelName">{test_type}</span>.
    </p>
    ...
    <div className="mt-6">
      <span className="bg-yellow-500 text-black px-4 py-2 rounded-full text-lg font-semibold">
        Level <span id="levelNumber"></span> Completed
      </span>
    </div>
    ...
    <div className="flex justify-between items-center mt-10 px-10">
      <div className="text-center">
        ...
        <h1 className='underline text-black'>Davronov.SH</h1>
      </div>
      <div className="text-center">
        <h3 className='text-gray-600'>Date:</h3>
        <p className="text-black mt-2 underline">{date+"/"+month+"/"+year}</p>
      </div>
    </div>
